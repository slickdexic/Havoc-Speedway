...
**Game Design Reference Document - Version 2.0**
...
7. [Animation and Timing Specifications](#animation-and-timing-specifications)
...
3. **Initial Setup**:
   - Deal cards face down to each player.
     - one card per player at a time, in order, until each player has the correct number of cards.
     (You summarize by removing information the needs to remain in some form. I want to keep the how, not just what.)
...
4. **Next round (Storm)**: Dealer button advances clockwise
...
- **Track Layout**: Position 96 is just before the start/finish line. Position 1 is just after the start/finish line (Thi belongs in its own section, not here)
...
- Pawns will travel across space util they "land on" a space.
- Coins merely travelled across are not trigerred.
...
- Crash into pit-lane wall end wall. (ex. Roll a 6 from the pit)
...
- Drawn coins appear face-up to self and cocieled (face-down) to others, below pawn (half extending below card edge)
...
- **Placement**: Selected coin disappears from card when placed on track
  - Coins are placed on track face down to conceal the coin's value
  - Player that place dhe coin can see that coins value on the track.
...
- **Player Colors**: Auto-assigned randomly from available unused colors on join
  ...
  - Change color button - select from available (unassigned colors) (why did you remove this? stop making ambiguous decisions.)
- **Host Controls**: Kick button - kick a player from the room
- **Messaging**: Private message to individual players, room chat (you made this too vague. How is this implemented? Don't on the door to misinterpretation.)
...
5. **Queen Play**: Chat bubble appears for suit selection
6. **Seven Play**: Draw pile changes to toxic color, text updates to "Draw 2"
...
- **Continuous Track**: Numbered sequentially
...
*Document Status: Comprehensive reference for implementation - All major systems documented with sufficient detail*
  (wrong. There is much more that I will add to this file, such as track layout, animations, sounds ...)
